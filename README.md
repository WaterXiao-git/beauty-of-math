...
- 已实现实验候选评分和 `direct`、`suggest`、`ai`、`answer`、`no-match` 五类路由决策。
- 已处理实验标题意图污染、长短标题冲突和常见自然语言插词；支持操作意图与数学知识文本拆分，以及核心标题、别名、关键词与文本相似度的渐进式匹配。
- 已将候选标记为 `exact`、`strong`、`related` 三档；相近文本命中只能推荐，不能触发自动跳转。
- 已在前端首页接入“智能实验导航”输入框，支持直接跳转、实验建议、待确认、数学解释和无匹配五类交互。
- 已提供 Agent HTTP 接口和对应的 Node.js 自动化测试。
- 已接入规则优先的双模型 Agent：`deepseek-v4-flash` 负责主路由；`qwen3.7-plus` 复核“已有可靠候选但决策置信度较低”的结果，并在主模型异常时接管。
- 已为已有实验路由增加 JSON 结构校验、实验 ID 白名单和安全降级；AI 不能编造已有页面 ID 或路径。
- 已支持从自然语言中提取已有实验的初始参数；例如“用方块演示 23 - 8”会打开原有四则运算页，并自动设置运算类型和数字。
- 已增加公式、图形名称和教学表现方式的结构化召回；无需输入完整实验标题，“把 3/4 画成饼图”“展示洛伦兹吸引子的蝴蝶效应”“给图做广度优先搜索”等口语表达也能命中对应专用页面。
- 已接通人工确认后的临时实验生成：没有合适预设实验时，可由 DeepSeek 生成配置、千问复核，并交给统一渲染页执行，不写入源码或永久路由。
- 临时实验页复用现有 300 个实验的侧栏、卡片、公式、参数面板、步骤控制和知识点布局，当前支持直角坐标函数、极坐标曲线、基础四则运算和沙箱 HTML/SVG/Canvas 四类渲染器。
- 已为高维超立方体投影提供本地可靠模板；“画一个四维超立方体的三维投影”等问题无需等待模型生成即可得到可调维度、旋转角和透视强度的交互实验。
- 生成入口只对具有可视化、表达式或数学模拟特征的问题开放；定义、含义和知识讲解类数学问题会进入 Agent 数学解释模式，天气和普通闲聊不会进入实验生成。
- 数学解释使用主模型生成标题、摘要、要点和示例，主模型异常时由副模型接管，不会为了复核而重复增加正常请求的等待时间。
- 完整实验配置使用独立的模型输出预算，并由本地规则补充二次项系数取零等关键退化情况，避免复杂配置被截断或遗漏课堂边界。
- 已优化路由响应链：明确命中和非数学问题不调用模型，真正未命中默认只请求一次主模型，不再为了显示确认入口串行等待副模型；双模型复核仍保留在可能影响已有候选选择的低置信度场景。

下一步将根据真实需求扩充可靠模板和生成能力，再补充内容审核、沙箱加固与离线评测，并把通过教师审核的临时实验转成可维护的正式实验模块。
...
| Agent 路由与生成 | 规则意图分类、实验注册表、候选评分、参数提取、数学解释、DeepSeek 主路由/生成、千问复核与故障接管 |
...
- 普通路由调用默认超时均为 4500 毫秒，可分别通过 `DEEPSEEK_TIMEOUT_MS`、`QWEN_TIMEOUT_MS` 调整；临时实验生成至少保留 20 秒，以便输出和复核完整配置。

模型调用只发生在歧义候选、相近候选、混合意图、数学知识解释或数学实验未命中场景。完整标题、强语义和高精度结构特征已经产生明确结果时继续使用本地规则；天气和普通闲聊会在本地直接拒绝，不增加外部调用延迟和费用。未命中时 DeepSeek 先判断是否应推荐、解释或生成实验；只有主模型异常，或已有可靠候选仍需要低置信度裁决时，才调用千问。
...
- `answer`：问题属于数学定义、含义或知识讲解，直接展示 Agent 的结构化解释、要点和示例。
- `no-match`：未识别到相关数学实验。

Agent 始终先运行本地规则和结构化召回。只有结果不明确时，DeepSeek 才会输出结构化路由建议；当已有可靠候选但模型决策置信度较低时，再由千问复核。未命中、追问和工具确认不会重复串行等待副模型，主模型超时或异常时仍由千问接管；两个模型都失败时保留原规则结果。
...
临时实验统一使用 `DynamicExperimentSpec` 描述页面标题、公式、参数、步骤、知识点和渲染器。前三类结构化渲染器继续校验表达式和采样范围；通用 HTML/SVG/Canvas 文档在无同源权限的 iframe 中运行，并通过消息同步外部参数。前端在写入会话存储和渲染前仍会再次校验；配置只在当前浏览器会话中使用，不会自动创建文件、修改 300 个正式实验或注册永久路由。当前阶段优先覆盖产品功能，沙箱策略和数学内容审核仍需在正式上线前继续加固。
...
- 为常见几何构造、统计图和数值模拟补充经过测试的可靠生成模板，并继续加固通用画布沙箱。
...
- AI 生成结果必须经过 Schema 校验；通用画布内容仅在隔离 iframe 中运行，不能直接写入项目源码或正式路由。
