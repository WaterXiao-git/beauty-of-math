      ...
      <div className="border-b border-slate-100 px-4 py-4">
        ...
        {courses.length > 0 && (
          <label className="mt-3 block">
            <span className="mb-1.5 block text-[11px] font-medium text-slate-400">当前课程</span>
            <select
              value={selectedCourseId}
              onChange={(event) => onSelectCourse?.(event.target.value)}
              className="h-10 w-full rounded-xl border border-blue-200 bg-blue-50 px-3 text-xs font-semibold text-blue-700 outline-none transition focus:border-blue-400 focus:ring-2 focus:ring-blue-100"
            >
              {courses.map((course) => (
