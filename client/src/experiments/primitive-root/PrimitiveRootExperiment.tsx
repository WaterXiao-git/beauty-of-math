...
<div className="grid grid-cols-5 gap-1.5">
                {Array.from({ length: p - 2 }, (_, i) => i + 2).map((v) => (
                  <button
                    key={v}
                    onClick={() => setG(v)}
                    className={`px-2 py-1.5 rounded text-sm font-medium ${g === v ? 'bg-purple-500 text-white' : isPrimitiveRoot(v, p) ? 'bg-indigo-100 text-indigo-700 hover:bg-indigo-200' : 'bg-gray-100 text-slate-500 hover:bg-gray-200'}`}
                  >
                    {v}
                  </button>
                ))}
