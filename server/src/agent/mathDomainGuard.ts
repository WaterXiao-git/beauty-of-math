...
const MATH_TERM_PATTERN = /(?:数学|算术|数轴|加法|减法|乘法|除法|分数|小数|比例|方程|不等式|函数|图像|曲线|曲面|坐标|极坐标|参数|几何|三角|圆|椭圆|抛物线|双曲线|向量|矩阵|行列式|概率|统计|分布|积分|微分|导数|极限|级数|数列|集合|组合|排列|图论|算法|定理|公式|轨迹|斜率|面积|体积|角度|对称|变换|收敛|发散|拟合|回归|聚类|拓扑|分形|混沌)/i
...
const VISUAL_REQUEST_PATTERN = /(?:可视化|画出|绘制|作图|图像|图形|曲线|曲面|坐标|轨迹|动画|演示|模拟|参数.{0,8}变化|交互实验|动态实验|方块|数轴)/i
