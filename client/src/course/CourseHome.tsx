...
import { courseChapters as localHigherMathematicsChapters } from './courseData'
import { collectCoursePoints, collectRelatedPoints, findChapterInCourse, findSectionInCourse, HIGHER_MATHEMATICS_COURSE } from './courseCatalog'
...
export default function CourseHome() {
  ...
  const course = useMemo(() => ({ ...HIGHER_MATHEMATICS_COURSE, chapters: higherMathematicsChapters }), [higherMathematicsChapters])
  const allPoints = useMemo(() => collectCoursePoints(course), [course])
  const point = selectedPointId
    ? allPoints.find((candidate) => candidate.id === selectedPointId || candidate.rendererId === selectedPointId)
    : undefined
  ...
  const breadcrumb = point ? ['首页', chapter?.title, section?.title, point.title].filter((item): item is string => Boolean(item)) : ['首页']
  const handleBreadcrumbClick = (index: number) => {
    if (index === 0) showCourseOverview()
    else if (index === 1 && chapter) {
