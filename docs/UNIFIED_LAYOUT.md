...
> 目标：所有演示页画布的点/线颜色按语义统一，可拖拽性由「拖拽外圈动画 + cursor」表达，颜色本身保持语义一致。

| 元素 | 颜色 | 说明 |
|---|---|---|
| 函数曲线 | `#60a5fa` 亮蓝 | 全局统一 |
| **顶点 / 极值点** | `#fb7185` 玫瑰 | 二次/绝对值顶点（可拖，反解参数） |
| **零点 / 根** | `#34d399` 翡翠绿 | 与 x 轴交点（静态标注） |
| **y 截距点** | `#c084fc` 紫 | 线性页可拖（改 b） |
| **x 截距点** | `#fbbf24` 琥珀 | 线性页可拖（改斜率） |
| **极限目标点 a** | `#2563eb` 蓝 | ε−δ 页可拖 |
| **P 点 / Q 点** | `#60a5fa` / `#fbbf24` | 导数页，均沿曲线可拖 |
| **中值点 ξ** | `#c084fc` 紫（`#34d399` 绿圈） | 罗尔页 |
| **割线 / 切线** | `#38bdf8` 天蓝 / `#f472b6` 粉 | 导数页贯穿线 |
| **对称轴 / 辅助虚线** | `#94a3b8` 灰虚线 | 二次对称轴、罗尔等高线等 |
| ε 带 / δ 带 | `#7dd3fc` / `#fcd34d` 淡色 | 半透明区域 |

约定：
1. 新增演示页优先复用上述语义色，不引入新色
2. 可拖拽点统一：拖拽中显示外圈动画（GeoPoint state==='drag'），命中区 r=16
3. 标注文字跟随点色（浅色调，如玫瑰 `#fda4af`、绿 `#6ee7b7`）
