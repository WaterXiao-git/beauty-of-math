...
![MathViz 智能实验导航技术栈与运行流程](./数学教学可视化系统-技术栈架构图.png)

可编辑的矢量源图见 [`数学教学可视化系统-技术栈架构图.svg`](./数学教学可视化系统-技术栈架构图.svg)。
...
- 课程首页已改为“课程概览—知识点工作区”两级结构，章节目录采用不挤压主体的隐藏抽屉；知识点选择写入 URL，支持浏览器前进、后退与链接分享。
- 首页实验库通过 `/api/content/experiments` 浏览、搜索和筛选 300 个正式实验；已迁移知识点使用后端发布版本，未迁移内容自动使用本地课程数据回退。
...
- 数学定义、概念含义和原理类问题可以直接由 Agent 输出结构化解释，不会被强行路由到不相关实验。
...
| 教学画板 | geoboard、SVG 视口、拖拽约束、平移缩放 |
...
| 输入问题 | 预期展示 | 核心能力 |
| --- | --- | --- |
| `用方块演示 23 - 8` | 打开加减乘除可视化，并自动设置减法、23 和 8 | 已有实验匹配与参数提取 |
| `我想看看椭圆、双曲线和抛物线有什么区别` | 推荐或打开圆锥曲线实验 | 口语化语义匹配 |
| `展示洛伦兹吸引子的蝴蝶效应` | 匹配洛伦兹吸引子实验 | 数学知识与表现方式联合召回 |
| `什么是群论中的群？` | 显示概念摘要、关键点和例子 | Agent 数学解释 |
| `画一个四维超立方体的三维投影` | 确认后生成可调维度、旋转角和透视强度的临时实验 | 未命中后的可靠模板生成 |

推荐按“参数化已有实验 → 语义匹配 → 数学解释 → 生成新实验”的顺序向客户演示，能够完整体现该入口不是普通搜索框，而是一个规则优先、Agent 增强的数学教学任务路由器。

## 核心页面

| 路由 | 用途 |
| --- | --- |
| `/` | 可点击返回的课程概览、章节抽屉与 300 个实验目录 |
| `/?point=:pointId` | 知识点工作区：知识地图、全课程知识点导航和知识点详情 |
| `/ask` | AI 智能提问与五分支路由结果 |
| `/demo/:pointId` | 统一知识点演示容器，当前包含罗尔定理、ε−δ 和导数演示 |
| `/temp/:specId` | 仅当前会话有效的临时交互实验预览 |
| 原有实验路由 | 继续保留 300 个已有实验页，例如 `/calculus`、`/fourier` |
...
│     ├─ contexts/                 # React 上下文
│     ├─ course/                   # 课程、章节、知识地图和详情界面
│     ├─ ask/                      # AI 提问与路由结果界面
│     ├─ demo/                     # 统一演示、临时实验与 geoboard
...
```bash
git clone https://github.com/WaterXiao-git/mathviz.git
cd mathviz
npm run install:all
```
...
| `POST` | `/api/route` | 新版 AI 提问页使用的规则优先路由兼容接口 |
