...
模型调用只发生在歧义候选、相近候选、混合意图或数学实验未命中场景。完整标题、强语义和高精度结构特征已经产生明确结果时继续使用本地规则；天气和普通闲聊会在本地直接拒绝，不增加外部调用延迟和费用。未命中时 DeepSeek 先判断是否存在可安全推荐或生成的实验，只有主模型异常，或已有可靠候选仍需要低置信度裁决时，才调用千问。
