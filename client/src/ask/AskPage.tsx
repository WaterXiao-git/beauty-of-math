...
function pointDestination(point: KnowledgePoint): string {
  if (point.demoId) return `/demo/${point.demoId}`
  return point.experimentPath ?? `/experiments?q=${encodeURIComponent(point.title)}`
}
...
function resolveRoutePoint(match?: RouteMatch): KnowledgePoint | undefined {
  ...
  return (
    ...
    ALL_POINTS.find((point) => point.experimentPath === match.path) ??
...
export default function AskPage() {
  ...
  const navigationCourse = findCourse(navigationCourseOverride ?? contextCourse?.id ?? DEFAULT_COURSE_ID)
  ...
  const mainMatch = contextPoint || top
    ? {
        ...
        experimentPath: contextPoint ? pointDestination(contextPoint) : top?.path,
        templates: [
  ...
  const handleEnterDemo = () => {
    if (mainMatch?.experimentPath) navigate(mainMatch.experimentPath)
  }
  ...
  return (
    <div className="flex flex-col h-full bg-[#f5f7fa]">
      ...
      <DrawerSidebar
        ...
        onSelectPoint={(pointId) => navigate(`/?course=${encodeURIComponent(navigationCourse.id)}&point=${encodeURIComponent(pointId)}`)}
